...
struct RandomClustering <: AMM end

const CLUSTERING_ALGORITHMS = [KMeans(), GMM(), RandomClustering()]
...
function em(
    alg::RandomClustering,
    x::Matrix{<:Real};
    k::Int=3,
    kwargs...
)::Tuple{Vector,Vector}
    #=
    Dummy returns for consistency with the "normal" `em` method.
    Empty parameter history.
    Cluster assignments at random (same signature as for `KMeans`)
    =#
    return [], [[rand(1:k) for _ in 1:size(x)[1]]]
end
